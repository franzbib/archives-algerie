...
const DEFAULT_PROMPT_PATH = "prompts/ASSISTED_READING_PROMPT.md";
const DEFAULT_OUTPUT_DIRECTORY = ".local/archive-sample/assisted-reading";
...
async function main() {
  ...
  if (!confirmed) {
    throw new Error(
      "Generation refusee: ajoutez --confirm pour confirmer l'appel OpenAI et l'ecriture locale.",
    );
  }

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    throw new Error("OPENAI_API_KEY est requise pour generer une lecture assistee.");
  }

  const inputPath = getRequiredArg("--input");
  const sourceImage = getRequiredArg("--source-image");
  ...
  await requireFile(promptPath, "prompt de lecture assistee");
  ...
  const generated = await generateAssistedReading({
    apiKey,
    model,
    prompt,
  });
  ...
  console.log(`Assisted reading written: ${outputPath}`);
}

async function generateAssistedReading(options: {
  apiKey: string;
  model: string;
  prompt: string;
}): Promise<unknown> {
  ...
  if (!response.ok) {
    throw new Error(
      `Erreur OpenAI ${response.status}: ${await response.text()}`,
    );
  }
...
function buildPrompt(options: {
  ...
  return options.promptTemplate
    ...
    .replaceAll("{{rawOcrText}}", options.rawOcrText)
...
function getDefaultOutputPath(inputPath: string): string {
  ...
  return path.join(DEFAULT_OUTPUT_DIRECTORY, `${baseName}.assisted.json`);
}
