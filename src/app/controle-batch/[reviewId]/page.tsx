...
import {
  getAssistedReadingForBatchReview,
  getBatchReviewItemById,
  getBatchReviewItems,
  getPublicBatchAssetForReview,
...
export function generateStaticParams() {
  return getBatchReviewItems().map((item) => ({ reviewId: item.reviewId }));
}
...
export default async function BatchReviewPage({
  ...
  const reviewItem = getBatchReviewItemById(reviewId);

  if (!reviewItem) {
    notFound();
  }

  const asset = getPublicBatchAssetForReview(reviewItem);

  if (!asset) {
    notFound();
  }

  const assistedReading = getAssistedReadingForBatchReview(reviewItem);
      ...
      <div className="border-b border-paper-border bg-paper/60">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-4 px-6 py-4 lg:px-8">
          ...
          <div className="flex flex-1 items-center justify-end gap-6 text-sm font-medium">
            ...
            {nextItem ? (
              <Link
                href={`/controle-batch/${nextItem.reviewId}`}
                className="text-foreground hover:text-warm"
              >
                Suivant &rarr;
              </Link>
      ...
      <section className="border-b border-paper-border bg-paper">
        <div className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
          <p className="font-mono text-xs font-semibold uppercase tracking-widest text-warm">
            Controle detaille du lot complet
          </p>
          <h1 className="mt-3 font-serif text-4xl font-medium text-foreground md:text-5xl">
            Image batch {formatReviewTitle(reviewItem.reviewId)}
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-7 text-foreground/80">
            Cette page rapproche une image publiee sur R2 de sa lecture assistee
            non validee. Elle ne publie pas les OCR locaux et ne valide ni la
            page, ni le document, ni la transcription.
          </p>
        </div>
      </section>
  ...
  return (
    <section className="border border-paper-border bg-paper">
      <div className="border-b border-paper-border bg-background/60 px-5 py-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2 sm:justify-end">
            <StatusBadge variant="warning">Image batch non validee</StatusBadge>
      ...
      <div className="bg-background p-4">
        ...
        <img
          alt={`Image batch ${reviewItem.reviewId} - ${asset.localJpgFileName}`}
      ...
      <div className="space-y-5 px-5 py-5">
        ...
        <div className="border border-paper-border bg-background p-4 text-sm leading-6 text-foreground/80">
          ...
          <p className="mt-2">
            Cette image du lot complet n&apos;est pas encore validee comme page
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <div className="mt-4 space-y-2 text-sm leading-6 text-foreground/80">
            <p>Aucune lecture assistee n&apos;est reliee a cette image.</p>
            <p>
          ...
          <div className="mt-3 space-y-2 text-sm leading-6 text-foreground/80">
            ...
            <p>
              {assistedReading
                ...
                : "Lecture assistee non disponible pour cette image du batch."}
            </p>
    ...
    <section className="border border-paper-border bg-paper">
      ...
      <div className="p-5">
        ...
        <p className="mb-4 text-sm leading-6 text-foreground/80">
          Les elements suivants doivent etre controles avant toute citation ou
          indexation historique :
        </p>

        <ul className="mb-6 space-y-2 text-sm font-medium text-foreground/90">
          <li>Noms propres et patronymes</li>
          <li>Lieux et toponymes</li>
          <li>Dates et chronologie</li>
          <li>Sigles et abreviations</li>
        </ul>

        <div className="border border-paper-border bg-background p-4 text-sm leading-6 text-foreground/75">
          <p>Aucun workflow d&apos;edition n&apos;est actif dans cette etape batch.</p>
        </div>
      </div>
    </section>
  );
